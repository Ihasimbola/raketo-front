...
function ItemList({ variant, data }: Props) {
  ...
  return (
    <div
      className={cn([listVariants({ variant }), "grid list"])}
      id={data._id}
      ref={rowRef}
    >
      <Text className="list__title">{data.title}</Text>
      <Text className="list__desc">{data.description}</Text>
      <div className="flex flex-row items-center gap-4 list__icons">
        <div
          onClick={startInterval}
          className={cn(
            "btn",
            chronoBtnActive === "play" ? "btn-desactive" : ""
          )}
        >
          <PlayIcon
            height={25}
        ...
        <div
          onClick={handleUpdateChrono}
          className={cn(
            "btn",
            chronoBtnActive === "pause" ? "btn-desactive" : ""
          )}
        >
          <PauseIcon
            height="25"
        ...
        <Text>
          {chrono.day ? <Text as="span">{chrono.day + "andro:"}</Text> : ""}
          <Text as="span">
            {chrono.hour < 10 ? "0" + chrono.hour.toString() : chrono.hour}
            <span className="mx-[3px]">:</span>
          </Text>
          <Text as="span">
            {chrono.minute < 10 ? "0" + chrono.minute : chrono.minute}
            <span className="mx-[3px]">:</span>
          </Text>
          <Text as="span">
        ...
        {isDone ? (
          <div className="flex justify-end ">
            <DoubleCheckIcon
              height="auto"
              width={40}
              currentColor="stroke-green-700"
            />
          </div>
        ) : (
          <fetcher.Form method="PATCH" action={`${data._id}`} className="">
            <Button type="submit" name="isDone" value="true">
              Vita
            </Button>
          </fetcher.Form>
        )}
